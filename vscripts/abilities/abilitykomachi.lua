...
function Komachi01_Think(keys)
	...
	if modifier:GetStackCount()<stackcount and caster:HasModifier("modifier_thdots_komachi_time_to_add_blink")==false then
		keys.ability:ApplyDataDrivenModifier(caster,caster,"modifier_thdots_komachi_time_to_add_blink",{duration = cooldown - FindTelentValue(caster,"special_bonus_unique_komachi")})
	end
end
...
function Komachi02_AttackLanded(keys)
	...
	local ability = caster:FindAbilityByName("ability_thdots_komachi02")
	if ability~=nil then
		local Damage = ability:GetAbilityDamage() + FindTelentValue(caster,"special_bonus_unique_komachi_2")
		for _,v in pairs(targets) do
			...
			-- end
				deal_damage = Damage*1.0
				Komachi03_FireEffect(caster,v)
			if FindTelentValue(caster,"special_bonus_unique_komachi_1") ~= 0 then
				Komachi03_FireEffect(caster,v)
			end
			local damage_table = {
	...
	if caster.komachi_03_targets ~= nil then
		for k,v in pairs(caster.komachi_03_targets) do
			if v:IsNull()==false and v ~= nil then
				...
				if modifier~=nil then
					local deal_damage = (keys.soul_damage + v:GetMaxHealth()*(keys.soul_percent_damage+FindTelentValue(caster,"special_bonus_unique_komachi_3"))/100 ) * modifier:GetStackCount()
					local damage_table = {
							ability = keys.ability,
						    victim = v,
						    attacker = caster,
...
function OnKomachi04SpellStart(keys)
	...
	target:SetContextThink(DoUniqueString("OnKomachi04SpellStart"), 
		function ()
			...
			if target:GetHealth() < ( target:GetMaxHealth() * (keys.KillHp + FindTelentValue(caster,"special_bonus_unique_komachi_4")) * 0.01 ) and target:IsAlive() and caster:IsAlive() then 
