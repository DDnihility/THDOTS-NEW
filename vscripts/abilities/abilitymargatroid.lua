...
function MargatroidEx_ModifyDollCount(caster,count)
	...
	local max = math.max(3,math.floor((caster:GetLevel()-1)/2+3)) + FindTelentValue(caster,"special_bonus_unique_margatroid_2")
...
function Margatroid_ModifyTotalDollNum(hCaster,iModifyAmount)
	...
	local max = math.max(3,math.floor((hCaster:GetLevel()-1)/2+3)) + FindTelentValue(hCaster,"special_bonus_unique_margatroid_2")
...
function Margatroid_ModifyUsableDollNum(hCaster,iModifyAmount)
	...
	if AbilityEx then
		...
		if stack_count > max then stack_count = max end
...
function MargatroidEx_OnDollDestroy(keys)
	...
	local Caster=keys.caster
	local doll=keys.target
	if doll:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_RECOVERING_NAME) then return end
	...
	doll:RemoveAllModifiers(0,true,true,true)
	doll:Destroy()
end
...
modifier_thdots_margatroid03_doll_Upgrate = {}  
LinkLuaModifier("modifier_thdots_margatroid03_doll_Upgrate","scripts/vscripts/abilities/abilitymargatroid.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_margatroid03_doll_Upgrate:IsHidden()       return true end
function modifier_thdots_margatroid03_doll_Upgrate:IsPurgable()     return false end
function modifier_thdots_margatroid03_doll_Upgrate:RemoveOnDeath()  return true end
function modifier_thdots_margatroid03_doll_Upgrate:IsDebuff()       return false end

function modifier_thdots_margatroid03_doll_Upgrate:DeclareFunctions()
    local funcs = {
        MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,
    }
    return funcs
end
function modifier_thdots_margatroid03_doll_Upgrate:OnCreated()
	if not IsServer() then return end
	self:StartIntervalThink(FrameTime())
end
function modifier_thdots_margatroid03_doll_Upgrate:OnIntervalThink()
	if not IsServer() then return end
	self:SetStackCount(self:GetCaster():GetDisplayAttackSpeed())
end
function modifier_thdots_margatroid03_doll_Upgrate:GetModifierAttackSpeedBonus_Constant()
...
function Margatroid03_OnSpellStart(keys)
	...
	MargatroidEx_ModifyDollCount(Caster,1)
	THDReduceCooldown(keys.ability, FindTelentValue(Caster,"special_bonus_unique_margatroid_6"))
	Ability:ApplyDataDrivenModifier(Caster,doll,MARGATROID_ABILITY03_MODIFIER_DOLL_NAME,{})
	if FindTelentValue(Caster,"special_bonus_unique_margatroid_5")~=0 then
		print(Caster:GetDisplayAttackSpeed())
		print(Caster:GetIncreasedAttackSpeed())
		print(Caster:GetAttackSpeed())
		doll:AddNewModifier(Caster,Ability,"modifier_thdots_margatroid03_doll_Upgrate",{})
	end
	...
	doll:SetBaseDamageMax(keys.DollDamage*(1+FindTelentValue(Caster,"special_bonus_unique_margatroid_3")))
	doll:SetBaseDamageMin(keys.DollDamage*(1+FindTelentValue(Caster,"special_bonus_unique_margatroid_3")))
	doll:SetMana(400)
	doll:SetControllableByPlayer(Caster:GetPlayerID(),false)
	doll:SetBaseMoveSpeed(400)
	if FindTelentValue(Caster,"special_bonus_unique_margatroid_5")==1 then
		doll:SetPhysicalArmorBaseValue(20)
		doll:SetBaseMagicalResistanceValue(20)
	end
	-- doll:SetHealth(doll:GetMaxHealth())
...
function Margatroid03_DollOnIntervalThink(keys)
	...
	if FindTelentValue(Caster,"special_bonus_unique_margatroid_0")==0 then
...
function Margatroid03_OnDollDestroy(keys)
	...
	local all_modifiers = doll:FindAllModifiers()
	for _, buff in pairs(all_modifiers) do
		if buff and not buff:IsNull() then
			local name = buff:GetName()
			if doll:HasModifier(name) and name ~= "modifier_thdots_margatroid03_doll" and name ~= "modifier_kill" then
				doll:RemoveModifierByName(name)
			end
		end
	end
	-- doll:RemoveAllModifiers(0, true, true, true)
...
function Margatroid33_KillItself(keys)
	...
	-- keys.caster:RemoveAllModifiers(0, true, true, false)
	-- keys.caster:Destroy()
	Margatroid03_OnDollDestroy({
		ability = keys.ability,
		caster = keys.caster.ability_alice_03_owner,
		target = keys.caster
	})
end
...
function Margatroid04_DamageTargets(keys)
	...
	for k,v in pairs(targets) do
		UnitDamageTarget{
			...
			victim = v,
			attacker = Caster,
			damage = keys.explosion_damage + FindTelentValue(Caster,"special_bonus_unique_margatroid_4"),
			damage_type = Ability:GetAbilityDamageType()
		}
	end
end