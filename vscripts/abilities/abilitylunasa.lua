	...
	local targetPoint = caster:GetOrigin()
 	...
 	for _,v in pairs(targets) do
 		local DamageTable = {
			                ...
			                victim = v, 
			                attacker = caster, 
			                damage = keys.damage + caster:GetIntellect()*keys.ability:GetSpecialValueFor("int_bonus"), 
...
function lunasa02OnspellStart(keys)
	...
	if not target:IsBuilding() and caster:GetTeam() ~= target:GetTeam() then 
		...
		if not lunasaModifier then
			...
			target:SetModifierStackCount("modifier_lunasa02_debuff", keys.ability,1 + FindTelentValue(caster,"special_bonus_unique_lunasa_2"))
		else
			local count = target:GetModifierStackCount("modifier_lunasa02_debuff", nil)
			lunasaModifier:SetDuration(duration, true)
			target:SetModifierStackCount("modifier_lunasa02_debuff", keys.ability, count + 1 + FindTelentValue(caster,"special_bonus_unique_lunasa_2"))
			if count >= 16 then 
...
function ability_thdots_lunasa03:GetIntrinsicModifierName()
	return "modifier_lunasa03"
end

-- 随着生命值减少,法术暴击提升
modifier_lunasa03 = {}
LinkLuaModifier("modifier_lunasa03","scripts/vscripts/abilities/abilitylunasa.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_lunasa03:IsHidden() 		return false end
function modifier_lunasa03:IsPurgable()		return false end
function modifier_lunasa03:RemoveOnDeath() 	return false end
function modifier_lunasa03:IsDebuff()		return false end
function modifier_lunasa03:OnCreated()
	if not IsServer() then return end
	self:StartIntervalThink(FrameTime())
end
function modifier_lunasa03:OnIntervalThink()
	if not IsServer() then return end
	local caster = self:GetCaster()
...
-- 	local LUNASA_DAMAGE_BONUS_PERCENT = (caster:GetMaxHealth() - caster:GetHealth())/caster:GetMaxHealth() * 10 / (keys.AbilityMulti - FindTelentValue(caster,"special_bonus_unique_lunasa_4"))
