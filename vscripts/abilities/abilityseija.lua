...
function ability_thdots_seija02:OnProjectileHitHandle(hTarget, vLocation, iProjectileHandle)
	...
	target:AddNewModifier(caster,ability, "modifier_ability_thdots_seija02_debuff",{duration = self.duration_slow * (1 - self:GetParent():GetStatusResistance())})
	local damage_dealt = UnitDamageTarget(damageTable)
	for i = 1,#self.projectile_table do
		if self.projectile_table[i].barrage == iProjectileHandle then
...
function modifier_ability_thdots_seija02_debuff:GetModifierMoveSpeedBonus_Percentage()
	if not IsServer() then return self:GetAbility():GetSpecialValueFor("movement_slow") end
...
function ability_thdots_seija03:OnSpellStart()
	...
	if target:IsHero() then
		...
		if self.intellect_all < self.attack then
			target:AddNewModifier(caster, self, "modifier_ability_thdots_seija03_sign",{duration = duration * (1 - target:GetStatusResistance())})
		end
	end
	target:AddNewModifier(caster, self, "modifier_ability_thdots_seija03",{duration = duration * (1 - target:GetStatusResistance())})
...
function modifier_ability_thdots_seija03_passive:OnAttackLanded(kv)
	...
	if kv.attacker == caster and target:IsHero() then
		...
		if caster_int < target_int and target and not target:IsNull() and target:IsAlive() then --造成智力差距的魔法伤害
			local damage_tabel = ({
				victim 			= target,
				-- Damage starts ramping from when cast time starts, so just gonna simiulate the effects by adding the cast point
...
function modifier_ability_thdots_seija04:OnDestroy()
	if not IsServer() then return end
