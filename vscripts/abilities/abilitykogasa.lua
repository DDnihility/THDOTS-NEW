...
function Kogasa02OnThink( keys )
	...
	local speed = FindValueTHD("critical_speed",ability)/caster:GetDisplayAttackSpeed()-- critical_speed是转速常数，值越大伤害越低
...
function Kogasa04OnThink( keys )
	...
	elseif Kogasa04_count >= preparetime then
		...
		if #target > 0 then
			if caster:HasModifier("modifier_item_wanbaochui") then
				keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_ability_thdots_kogasa04_wanbaochui", {})
			end
			...
			for _,v in pairs(targets) do
				...
				UnitDamageTarget(damage_table)
				if v and not v:IsNull() and v:IsRealHero() then
			...
			local effectIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_earthshaker/earthshaker_echoslam_start.vpcf", PATTACH_CUSTOMORIGIN,nil)
