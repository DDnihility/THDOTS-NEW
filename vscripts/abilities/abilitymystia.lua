...
function Mystia02OnSpellStart(keys)
    ...
    local duration = keys.ability:GetSpecialValueFor("duration")
    local caster = EntIndexToHScript(keys.caster_entindex)
    -- local flag = FindTelentValue(caster,"special_bonus_unique_mystia_4")
...
function Mystia04OnSpellStart(keys)
    ...
    --对于英雄单位
    if v and not v:IsNull() and v:IsHero() then
...
function mystiaExOnSpellStart( keys )
  ...
  keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_mystiaEx_fly", {Duration = 4 + FindTelentValue(caster,"special_bonus_unique_mystia_4")})
end
  ...
  if target:IsAncient() then 
    target_level = target_level * 2
      ...
      if caster_level > target_level then 
        ...
        --10级天赋
        if FindTelentValue(caster,"special_bonus_unique_mystia_3") ~= 0 then
           damage = damage * 2
        end
        print(damage)
        local damage_table = {
