...
function modifier_ability_thdots_keineEx_passive_aura:GetModifierPercentageRespawnTime()
	...
	if self:GetCaster():HasModifier("modifier_ability_thdots_keineEx_talent3") then
		return self:GetAbility():GetSpecialValueFor("wanbaochui_percent_time") / 100
	else
		return 0
	end
end
...
function modifier_ability_thdots_keine01_sawa:OnDestroy()
	...
	if self:GetParent():IsAlive() then
		...
		if self:GetRemainingTime() < 0 then --被驱散就不生效
			...
			local final_health = ( total_health + change_health ) / 100
			--print("change_health = "..change_health)
			--print(math.abs(self.unit:GetMaxHealth() * change_health/100))
			if self.unit:IsMagicImmune()==true then return end --魔法免疫
			if change_health > 0 then
				self.unit:SetHealth(self.unit:GetMaxHealth() * final_health)
			else
				local damage_table = {
				ability = self:GetAbility(),
			    victim = self.unit,
			    attacker = self:GetCaster(),
			    damage = math.abs(self.unit:GetMaxHealth() * change_health/100),
			...
			SendOverheadEventMessage(nil, OVERHEAD_ALERT_MAGICAL_BLOCK, self:GetParent(), final_health, nil)
		end
	end
end
...
function ability_thdots_keine02:OnSpellStart()
	...
	local set_health = caster:GetHealth() - health
	--设置造成伤害以触发船勾
	local damage_table = {
				ability = self,
			    victim = caster,
	...
	if target:GetTeamNumber() == caster:GetTeamNumber() then
		...
		target:AddNewModifier(caster, self, "modifier_ability_thdots_keine02_debuff",{duration = debuff_duration * (1 - target:GetStatusResistance())})
	end
	--sound
...
function ability_thdots_keine03:OnSpellStart()
	...
	if target and not target:IsNull() then
		target:SetContextThink("keine03_kill",
			function ()
				if target:IsAlive()==false and target:IsRealHero() then
					self:EndCooldown()
...
function modifier_ability_thdots_keine04:OnCreated()
	...
	if caster:GetName() == "npc_dota_hero_void_spirit" then
		caster:SetModel("models/keine/keine.vmdl")
		caster:SetOriginalModel("models/keine2/keine2.vmdl")
	end
end
...
function modifier_ability_thdots_keine04:OnDestroy()
	...
	if caster:GetName() == "npc_dota_hero_void_spirit" then
		caster:SetModel("models/keine/keine.vmdl")
		caster:SetOriginalModel("models/keine/keine.vmdl")
	end
