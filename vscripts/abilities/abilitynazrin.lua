...
function ability_thdotsr_NazrinEx:OnSpellStart()
	...
	self.HASTE_threshold = self:GetSpecialValueFor("DOTA_RUNE_HASTE_chance") + self.BOUNTY_threshold
	self.ILLUSION_threshold = self:GetSpecialValueFor("DOTA_RUNE_ILLUSION_chance") + self.HASTE_threshold
	self.INVISIBILITY_threshold = self:GetSpecialValueFor("DOTA_RUNE_INVISIBILITY_chance") + self.ILLUSION_threshold
	self.REGENERATION_threshold = self:GetSpecialValueFor("DOTA_RUNE_REGENERATION_chance") + self.INVISIBILITY_threshold
	self.DOUBLEDAMAGE_threshold = self:GetSpecialValueFor("DOTA_RUNE_DOUBLEDAMAGE_chance") + self.REGENERATION_threshold
...
function ability_thdotsr_NazrinEx:OnChannelFinish(bInterrupted)
	...
	if not bInterrupted then
		...
		elseif random_int > self.BOUNTY_threshold and random_int <= self.HASTE_threshold then
			CreateRune(self:GetCursorPosition(), DOTA_RUNE_HASTE)
		elseif random_int > self.HASTE_threshold and random_int <= self.ILLUSION_threshold then
		...
		elseif random_int > self.INVISIBILITY_threshold and random_int <= self.REGENERATION_threshold then
			CreateRune(self:GetCursorPosition(), DOTA_RUNE_REGENERATION)
		elseif random_int > self.REGENERATION_threshold and random_int <= self.DOUBLEDAMAGE_threshold then
