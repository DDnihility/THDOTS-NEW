...
function OnPatchouliInvoke(keys)
	...
	--[[local manaCost = keys.ManaCost*0.01*caster:GetMana()		--消耗当前法力值*蓝耗系数
		if FindTelentValue(caster,"special_bonus_unique_patchouli_5") ~= 1 then
...
function OnPatchouliFireWood(keys)
	...
	if GridNav:IsNearbyTree(keys.target_points[1], keys.Radius+FindTelentValue(caster,"special_bonus_unique_patchouli_10"), false) then
		tree = 1
	end
...
function OnPatchouliFireMetal(keys)
	...
	if target:GetTeam()==caster:GetTeam() then
		...
		target:SetModifierStackCount("modifier_patchouli_fire_metal_bonus_damage", keys.ability, firelevel*15+FindTelentValue(caster,"special_bonus_unique_patchouli_1"))
		target:SetModifierStackCount("modifier_patchouli_fire_metal_bonus_attack_speed", keys.ability, metallevel*15+FindTelentValue(caster,"special_bonus_unique_patchouli_1"))
		keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_patchouli_fire_metal_effect", {})
	else	
		if is_spell_blocked(keys.target) then return end
		local deal_mana_damage = keys.ManaDamage + metallevel*keys.MetalMultiple + target:GetMana()*(keys.BaseManaDamage)
...
function OnPatchouliWoodEarth(keys)
	...
	local caster=keys.caster
	local target=keys.target
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_patchouli_3"))
...
function OnPatchouliMetalEarth(keys)
	...
	local deal_damage=keys.Damage+metallevel*keys.ExDamage+FindTelentValue(Caster,"special_bonus_unique_patchouli_9")
...
function MetalMetalOnCreated( keys )
	...
	keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_patchouli_metal_metal", {duration=10.0})--野蛮
end
...
function OnPatchouliXianzhezhishi(keys)
	...
	if caster:HasModifier("modifier_thdots_patchouli_xianzhezhishi_add")==false then caster.stone_count = 0 end
	caster.stone_count = caster.stone_count + 1
	if FindTelentValue(caster,"special_bonus_unique_patchouli_6") == 1 then
	...
	caster:SetModifierStackCount("modifier_thdots_patchouli_xianzhezhishi_add", ability_xianzhezhishi, caster.stone_count)
	if caster.stone_count == 5 then
		caster.stone_count = 0
		...
		if FindTelentValue(caster,"special_bonus_unique_patchouli_8") == 1 then			
