...
function ability_thdotsr_star02:OnSpellStart()
	...
	self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_ability_thdotsr_star02_curse", {duration=self:GetSpecialValueFor("snare_duration") * (1 - target:GetStatusResistance())})
	...
	if FindTelentValue(caster,"special_bonus_unique_star_3") ~= 0 then
		...
		for _,v in pairs(targets) do 
			if v ~= target then
				v:AddNewModifier(self:GetCaster(), self, "modifier_ability_thdotsr_star02_curse", {duration=self:GetSpecialValueFor("snare_duration") * (1 - v:GetStatusResistance())})
