...
function ability_thdots_reisen_2_01:GetAOERadius()
    return self:GetCastRange();
end

--修改cd：万宝槌
function ability_thdots_reisen_2_01:GetCooldown(iLevel)
...
function ability_thdots_reisen_2_01:OnSpellStart()
    ...
    if caster:HasModifier("modifier_item_wanbaochui") then 
        local particle = "particles/econ/items/windrunner/windranger_arcana/windranger_arcana_focusfire_start_tornado_sprite.vpcf"
        self.particle = ParticleManager:CreateParticle(particle, PATTACH_ABSORIGIN_FOLLOW, caster)     
    else
...
function ability_thdots_reisen_2_02:GetIntrinsicModifierName()
    return "modifier_ability_thdots_reisen2_02_charge"
end

function ability_thdots_reisen_2_02:GetAOERadius()
    return self:GetSpecialValueFor("radius");
end

--天赋：允许眩晕使用
function ability_thdots_reisen_2_02:GetBehavior()
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_ability_thdots_reisen2_04_telent_1") then
        return DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_IMMEDIATE + DOTA_ABILITY_BEHAVIOR_IGNORE_SILENCE + DOTA_ABILITY_BEHAVIOR_IGNORE_PSEUDO_QUEUE
    end
...
function ability_thdots_reisen_2_02:OnSpellStart()
    if not IsServer() then return end
    local caster=self:GetCaster()
    local ability_duration=self:GetSpecialValueFor("duration") + FindTelentValue(self:GetCaster(),"special_bonus_unique_Reisen_2_ability2_add_duration")
    local charge = caster:FindModifierByName("modifier_ability_thdots_reisen2_02_charge")

    print(ability_duration)
...
modifier_ability_thdots_reisen2_02_charge=class({})
LinkLuaModifier("modifier_ability_thdots_reisen2_02_charge","scripts/vscripts/abilities/abilityreisen2.lua",LUA_MODIFIER_MOTION_NONE)

--modifier 基础判定
function modifier_ability_thdots_reisen2_02_charge:IsHidden()
    if self:GetCaster():HasModifier("modifier_item_wanbaochui") then
        return false
    end
...
function modifier_ability_thdots_reisen2_02_charge:IsPurgable()       return false end
function modifier_ability_thdots_reisen2_02_charge:RemoveOnDeath()    return false end
function modifier_ability_thdots_reisen2_02_charge:IsDebuff()     return false end

function modifier_ability_thdots_reisen2_02_charge:OnCreated()
    if not IsServer() then return end
    self:StartIntervalThink(FrameTime())
end

function modifier_ability_thdots_reisen2_02_charge:OnDestroy()
    if not IsServer() then return end
    self:StartIntervalThink(-1)
end

function modifier_ability_thdots_reisen2_02_charge:OnIntervalThink()
    if not IsServer() then return end

    local caster = self:GetCaster()
    local ability = self:GetAbility()
    local charge = ability:GetSpecialValueFor("agh_charge")

    if self:IsHidden() == false then 
        if not caster:HasModifier("modifier_ability_thdots_reisen2_02_charge_think") and self:GetStackCount() < charge then 
            caster:AddNewModifier(caster, ability, "modifier_ability_thdots_reisen2_02_charge_think", {duration = ability:GetCooldown(ability:GetLevel()-1)})
    ...
    elseif self:IsHidden() and caster:HasModifier("modifier_ability_thdots_reisen2_02_charge_think") then 
        caster:RemoveModifierByName("modifier_ability_thdots_reisen2_02_charge_think")
...
--modifier 名字：充能计时
modifier_ability_thdots_reisen2_02_charge_think=class({})
LinkLuaModifier("modifier_ability_thdots_reisen2_02_charge_think","scripts/vscripts/abilities/abilityreisen2.lua",LUA_MODIFIER_MOTION_NONE)

--modifier 基础判定
function modifier_ability_thdots_reisen2_02_charge_think:IsHidden() return false end
function modifier_ability_thdots_reisen2_02_charge_think:IsPurgable()       return false end
function modifier_ability_thdots_reisen2_02_charge_think:RemoveOnDeath()    return false end
function modifier_ability_thdots_reisen2_02_charge_think:IsDebuff()     return false end

function modifier_ability_thdots_reisen2_02_charge_think:OnDestroy()
    if not IsServer() then return end

    local parentModifier = self:GetCaster():FindModifierByName("modifier_ability_thdots_reisen2_02_charge")
...
function modifier_ability_thdots_reisen2_02_buff_damageReduction:IsPurgable()       return true end
...
function modifier_ability_thdots_reisen2_03:OnAttacked(keys)
    ...
    if keys.target==self:GetParent() and not keys.target:HasModifier("modifier_illusion") and keys.attacker:IsHero() then
        --天赋判定：倍数
        -- if FindTelentValue(self:GetCaster(),"special_bonus_unique_Reisen_2_ability3_multiplier") ~= 0 then
...
function ability_thdots_reisen_2_ultimate:GetAOERadius()
    return self:GetSpecialValueFor("radius");
end
...
LinkLuaModifier("modifier_ability_thdots_reisen2_04_telent_1","scripts/vscripts/abilities/abilityreisen2.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_reisen2_04_telent_1:IsHidden()       return true end
function modifier_ability_thdots_reisen2_04_telent_1:IsPurgable()     return false end
function modifier_ability_thdots_reisen2_04_telent_1:RemoveOnDeath()  return false end
function modifier_ability_thdots_reisen2_04_telent_1:IsDebuff()       return false end
