function OnMinamitsu01SpellStart(keys)
	...
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_minamitsu_3"))
			...
			function ()
				...
				if distance < keys.range then
				    ...
				    for i = #targets, 1, -1 do
				    	if targets[i] == caster or targets[i]:GetClassname()=="npc_dota_roshan" or targets[i]:GetUnitName()=="ability_minamitsu_04_ship" 
				    		or targets[i]:HasModifier("dummy_unit") or targets[i]:HasModifier("modifier_item_dragon_star_buff") 
				    		or targets[i]:HasModifier("modifier_meirin02_buff") then
...
function OnMinamitsu02SpellStart(keys)
	...
	local effectIndex = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_02_body.vpcf", PATTACH_WORLDORIGIN, nil)
	ParticleManager:SetParticleControl(effectIndex, 3, vecHook)
			...
			function ()
			    ...
			    for k,v in pairs(targets) do
			    	if v:IsNull() == false and v~=nil then
						...
						UnitDamageTarget(damage_table)
						if v and not v:IsNull() and not v:HasModifier("modifier_item_dragon_star_buff") and not v:HasModifier("modifier_meirin02_buff") then
...
function OnMinamitsu03SpellStart(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_minamitsu_2"))
...
function OnMinamitsu03AttackLanded(keys)
	...
	if FindTelentValue(caster,"special_bonus_unique_minamitsu_4")==1 then
...
function OnMinamitsu04ShipDeath(keys)
	...
	if hero ~= nil and hero:IsNull() == false then
		...
		if ability ~= nil then
			ability:EndCooldown()
			ability:StartCooldown(keys.ability:GetCooldown(keys.ability:GetLevel()-1) + FindTelentValue(hero,"special_bonus_unique_minamitsu_1"))
		end
	end
end
...
function OnMinamitsu04ShipDestroy(keys)
	...
	if hero ~= nil and hero:IsNull() == false then
		...
		if ability ~= nil then
			if hero.IsDriving then
				OnMinamitsu04ShipDownload(hero)
				if FindTelentValue(caster,"special_bonus_unique_minamitsu_5")==0 then
