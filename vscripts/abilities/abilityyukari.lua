...
function Yukari02_OnSpellStart(keys)
	...
	local duration = keys.HeroHiddenDuration + FindTelentValue(caster, "special_bonus_unique_yukari_2")
	if Yukari_CanMovetoGap(target) then
...
function Yukari03_OnSpellStart(keys)
	local ability=keys.ability
	local caster=keys.caster
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_yukari_3"))
	local target=keys.target_points[1]
...
function Yukari04_OnSpellStart(keys)
	local ability=keys.ability
	local caster=keys.caster
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_yukari_4"))
	local target=keys.target_points[1]
	local vecPos=nil
