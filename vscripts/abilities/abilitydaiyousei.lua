...
function ability_thdots_daiyousei01:OnSpellStart()
	...
	local item_ertianyiliu_crit_chance = 0
	local item_ertianyiliu_crit_mult = 100
	for i=0,5 do 
	...
	for i=0,5 do 
		item = caster:GetItemInSlot(i)
		if item ~= nil and item:GetName() == "item_ertianyiliu" then
			item_ertianyiliu_crit_chance = item:GetSpecialValueFor("crit_chance")
			item_ertianyiliu_crit_mult = item:GetSpecialValueFor("crit_mult")
			break
		end
	end
	...
	elseif RollPercentage(item_ertianyiliu_crit_chance) then
		self.crit_mult = item_ertianyiliu_crit_mult
	end
...
function ability_thdots_daiyousei05:OnSpellStart()
	...
	local caster 				= self:GetCaster()
	local radius  				= self:GetSpecialValueFor("radius")
	local attack_crtl  			= self:GetSpecialValueFor("attack_crtl") / 100
	...
	for _,v in pairs(targets) do
		...
		-- if distance > damage_radius then
			...
			if v:IsHero() then 
				victim_damage = victim_damage + damage_hero
			end
