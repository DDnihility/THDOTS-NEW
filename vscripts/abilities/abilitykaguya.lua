...
function OnKaguya01SpellStart(keys)
	...
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_kaguya_1"))
	--设置计数器，控制旋转角度
...
function OnKaguya01SpellThink(keys)
	...
	for _,v in pairs(targets) do
		...
		local deal_damage = keys.ability:GetAbilityDamage() + FindTelentValue(caster,"special_bonus_unique_kaguya_2")+v.kaguya01_exdamage_count*keys.ExDamage
		...
		if v and not v:IsNull() and caster:HasModifier("modifier_item_wanbaochui") then
	...
	local damage_table_caster = {
			...
			victim = caster,
			attacker = caster,
			damage = keys.HealthCost/100 * caster:GetMaxHealth(),
			damage_type = keys.ability:GetAbilityDamageType(), 
			damage_flags = 0
	}
...
function OnKaguyaSwapAbility(keys)
	...
	if 	FindTelentValue(caster,"special_bonus_unique_kaguya_4")==1 then
...
function OnKaguya02SpellDamage(keys)
	...
	for _,v in pairs(targets) do
		local damage_table = {
			    ...
			    victim = v,
			    attacker = caster,
			    damage = (keys.AbilityDamage + FindTelentValue(caster,"special_bonus_unique_kaguya_3"))/5,
			    damage_type = keys.ability:GetAbilityDamageType(), 
	    	    damage_flags = 0
		}
		UnitDamageTarget(damage_table)
...
function OnKaguya03ManaRegen(keys)
	...
	if(GameRules:IsDaytime()==true)then
		if FindTelentValue(caster,"special_bonus_unique_kaguya_5")==1 then		
	...
	caster.kaguya04dummy:SetContextThink("killself",
		function ()
			if caster.kaguya04dummy ~= nil then
				caster.kaguya04dummy:ForceKill(false)
			end
		end, 
	5)
	local damage_table_caster = {
...
function OnKaguya04Passive(keys)
	...
	if(lostHp>0)then
		if FindTelentValue(caster,"special_bonus_unique_kaguya_6")==1 then
