function OnMomiji01Spawn(keys)
	...
	count = 2 + FindTelentValue(caster,"special_bonus_unique_momiji_1")
	...
	for i=1,count do
		...
		if FindTelentValue(caster,"special_bonus_unique_momiji_2") == 1 then 
		...
		"ability_momiji01_Spawn_unit_regen", 
			function ()
				...
				if GetDistanceBetweenTwoVec2D(caster:GetOrigin(),unit:GetOrigin()) <= keys.AuraDistance then
					-- if unit:FindModifierByName("modifier_momiji01_wolf_regen") == nil then
					-- 	keys.ability:ApplyDataDrivenModifier(caster, unit, "modifier_momiji01_wolf_regen", {})
					-- end
					...
					-- if unit:FindModifierByName("modifier_momiji01_wolf_regen") ~= nil then
					-- 	unit:RemoveModifierByName("modifier_momiji01_wolf_regen")
					-- end
					local abilityMomiji04 = unit:FindAbilityByName("ability_thdots_momiji04")
					if abilityMomiji04 ~= nil then
...
function OnMomiji02Start(keys)
	...
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_momiji_3"))
	...
	local effectIndex = ParticleManager:CreateParticle("particles/heroes/momiji/ping_waypoint_circle.vpcf", PATTACH_CUSTOMORIGIN, caster)
	ParticleManager:SetParticleControl(effectIndex, 0, keys.target_points[1])
	...
	keys.ability:ApplyDataDrivenModifier(caster,unit,"modifier_momiji02_intervalthink",{})
...
function OnMomiji02IntervalThink(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local targets = FindUnitsInRadius(
					   caster:GetTeam(),		--caster team
	...
	for k,v in pairs(targets) do
		v:AddNewModifier(caster,keys.ability,"modifier_momiji02_debuff",{duration = 1})
	end
end
modifier_momiji02_debuff = modifier_momiji02_debuff or class({})
LinkLuaModifier("modifier_momiji02_debuff","scripts/vscripts/abilities/abilitymomiji.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_momiji02_debuff:IsHidden() return false end
function modifier_momiji02_debuff:IsDebuff() return true end
function modifier_momiji02_debuff:IsPurgable() return true end

-- Modifier particle
--function modifier_momiji02_debuff:GetEffectName()
--	return "particles/items2_fx/orchid.vpcf"
--end

--function modifier_momiji02_debuff:GetEffectAttachType()
--	return PATTACH_OVERHEAD_FOLLOW
--end

-- Reset damage storage tracking, track debuff parameters to prevent errors if the item is unequipped
function modifier_momiji02_debuff:DeclareFunctions()
	return {
...
function modifier_momiji02_debuff:OnAttackStart(keys)
	if IsServer() then
		local owner = self:GetParent()
		-- If this unit is the target, grant the attacker a crit buff
		if owner == keys.target then
			local attacker = keys.attacker
			local modifier = "modifier_momiji02_crit"
			if FindTelentValue(self:GetCaster(),"special_bonus_unique_momiji_4")~=0 then
				modifier = "modifier_momiji02_criticalstrike"
			end
			self:GetAbility():ApplyDataDrivenModifier(self:GetCaster(),attacker,modifier,{duration = 1})
		end
	end
end
