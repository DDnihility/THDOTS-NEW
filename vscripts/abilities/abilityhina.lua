...
function modifier_ability_thdots_hinaEx_passive:OnIntervalThink()
	...
	if FindTelentValue(self.caster,"special_bonus_unique_hina_5") ~= 0 and not self.caster:HasModifier("modifier_ability_thdots_hinaEx_talent5") then
		self.caster:AddNewModifier(self.caster,self:GetAbility(),"modifier_ability_thdots_hinaEx_talent5",{})
	end
...
modifier_ability_thdots_hinaEx_talent5 = modifier_ability_thdots_hinaEx_talent5 or {}  --天赋监听
LinkLuaModifier("modifier_ability_thdots_hinaEx_talent5","scripts/vscripts/abilities/abilityhina.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_hinaEx_talent5:IsHidden() 		return true end
function modifier_ability_thdots_hinaEx_talent5:IsPurgable()		return false end
function modifier_ability_thdots_hinaEx_talent5:RemoveOnDeath() 	return false end
function modifier_ability_thdots_hinaEx_talent5:IsDebuff()		return false end
...
function modifier_ability_thdots_hina01_doll:RemoveOnDeath() 	return true end
...
function ability_thdots_hina02:CastFilterResultTarget(target)
	local caster = self:GetCaster()
...
function ability_thdots_hina02:OnSpellStart()
	...
	local bond_modifier = bond_target:AddNewModifier(caster, ability, modifier_bonds, {duration = duration * (1 - bond_target:GetStatusResistance())})
	table.insert(modifier_table, bond_modifier)
				
	table.insert(bond_table, bond_target)
	linked_units[bond_target:GetEntityIndex()] = true

	for link = 1, max_targets-1 do
...
function ability_thdots_hina05:OnSpellStart()
	if not IsServer() then return end
	local caster = self:GetCaster()
	local duration = self:GetSpecialValueFor("duration")
	self.total_angle = 0
	caster:AddNewModifier(caster, self, "modifier_ability_thdots_hina05", {duration = duration})
end

modifier_ability_thdots_hina05 = {}
LinkLuaModifier("modifier_ability_thdots_hina05","scripts/vscripts/abilities/abilityhina.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_hina05:IsHidden() 		return false end
function modifier_ability_thdots_hina05:IsPurgable()		return false end
function modifier_ability_thdots_hina05:RemoveOnDeath() 	return true end
function modifier_ability_thdots_hina05:IsDebuff()		return false end

function modifier_ability_thdots_hina05:GetEffectName()
	return "particles/units/heroes/hero_brewmaster/brewmaster_cyclone.vpcf"
end

function modifier_ability_thdots_hina05:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_ability_thdots_hina05:OnCreated()
	self.caster 				= self:GetParent()
	self.ability 				= self:GetAbility()
	if not IsServer() then return end
...
function modifier_ability_thdots_hina05:OnIntervalThink()
	if not IsServer() then return end
	...
	self.current_direction = self.caster:GetForwardVector()
	-- 计算角度差
	local angle = RotationDelta((VectorToAngles(self.previous_direction)), VectorToAngles(self.current_direction)).y
	--print(angle)
	--+为逆时针，加速选装
	if angle > 0 then
		--print(angle)
		self:GetAbility().total_angle = self:GetAbility().total_angle + angle
		self.buff_ratio  = self:GetAbility().total_angle/3000
		--变大
		caster:SetModelScale(1 + self.buff_ratio/2)
	end
end

function modifier_ability_thdots_hina05:OnDestroy()
	if not IsServer() then return end
	self.caster 				= self:GetParent()
	self.ability 				= self:GetAbility()
...
--算比例加成
	self.buff_ratio  = self:GetAbility().total_angle/3000
	if self.buff_ratio > 1 then self.buff_ratio = 1 end
	self:GetAbility().attack_speed = math.floor(self.buff_ratio * self.attack_speed)
	self:GetAbility().move_speed = math.floor(self.buff_ratio * self.move_speed)
--回血回蓝
	local health_regen = self.buff_ratio * self.caster:GetMaxHealth() * self.regen*0.01
	local mana_regen = self.buff_ratio * self.caster:GetMaxMana() * self.regen*0.01
	self.caster:Heal(health_regen, self.ability)
	SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,self.caster,health_regen,nil)
	self.caster:GiveMana(mana_regen)
	SendOverheadEventMessage(nil,OVERHEAD_ALERT_MANA_ADD,self.caster,mana_regen,nil)
--上buff
	self.caster:AddNewModifier(self.caster, self.ability, "modifier_ability_thdots_hina05_buff", {duration = self.buff_duration})
	self.caster:SetModifierStackCount("modifier_ability_thdots_hina05_buff", self.ability, self:GetAbility().attack_speed)

	
end

modifier_ability_thdots_hina05_buff = {}
LinkLuaModifier("modifier_ability_thdots_hina05_buff","scripts/vscripts/abilities/abilityhina.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_hina05_buff:IsHidden() 		return false end
function modifier_ability_thdots_hina05_buff:IsPurgable()		return false end
function modifier_ability_thdots_hina05_buff:RemoveOnDeath() 	return true end
function modifier_ability_thdots_hina05_buff:IsDebuff()			return false end

function modifier_ability_thdots_hina05_buff:OnDestroy()
	self.caster = self:GetParent()
	if not IsServer() then return end
	self.caster:SetModelScale(1.0)
end

function modifier_ability_thdots_hina05_buff:DeclareFunctions()
	return {
...
function modifier_ability_thdots_hina05_buff:GetModifierMoveSpeedBonus_Percentage()
	return self:GetStackCount()*self:GetAbility():GetSpecialValueFor("move_speed")/100
end

function modifier_ability_thdots_hina05_buff:GetModifierAttackSpeedBonus_Constant()
	return self:GetStackCount()*self:GetAbility():GetSpecialValueFor("attack_speed")/100
end
...
function ability_thdots_hina04:GetCooldown(level)
	if self:GetCaster():HasModifier("modifier_ability_thdots_hinaEx_talent5") then
