...
function OnSakuyaExSpellStart(keys)
	...
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_sakuya_2"))
...
function OnSakuya03SpellStart(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	THDReduceCooldown(keys.ability,FindTelentValue(caster,"special_bonus_unique_sakuya_3") * 1.25)
	local targetPoint = keys.target_points[1]
