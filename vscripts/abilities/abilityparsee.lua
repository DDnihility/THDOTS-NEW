...
function create_illusion(caster,self, illusion_origin, illusion_incoming_damage, illusion_outgoing_damage, illusion_duration)
	...
	local tmp = caster
	if caster:GetPlayerOwner():GetContext("PlayerIsBot") == 1 then
		tmp = nil
	end
...
function ability_thdots_parsee01:GetAOERadius()
...
function modifier_ability_thdots_parsee01_dummy:OnIntervalThink()
	...
	elseif t > math.pi*1.2 then
		...
		elseif GetDistanceBetweenTwoVec2D(dummy:GetAbsOrigin(),target:GetAbsOrigin()) < 50 then --击中距离判定
			...
			print(damage)
			local damage_targets = FindUnitsInRadius(caster:GetTeam(), target:GetAbsOrigin(), nil,radius,
					ability:GetAbilityTargetTeam(),ability:GetAbilityTargetType(),0,0,false)
			for _,vic in ipairs(damage_targets) do
				local damage_tabel = {
					victim 			= vic,
					damage 			= damage,
					damage_type		= ability:GetAbilityDamageType(),
...
function ability_thdots_parsee02:OnSpellStart()
	...
	caster:SetContextThink("Parsee02",
		function()
			...
			if number > 0 then
				AddFOWViewer(caster:GetTeamNumber(), point,radius,1,false)
...
function modifier_ability_thdots_parsee02_debuff:GetModifierMoveSpeedBonus_Percentage()
	return -self:GetAbility():GetSpecialValueFor("slow_percent")* (1 - self:GetParent():GetStatusResistance())
end

function modifier_ability_thdots_parsee02_debuff:GetModifierAttackSpeedBonus_Constant()
	return -self:GetAbility():GetSpecialValueFor("slow_attack")* (1 - self:GetParent():GetStatusResistance())
end
...
function modifier_ability_thdots_parsee04_dummy:OnCreated()
	...
	self.life_steal  	= self.ability:GetSpecialValueFor("life_steal")
	...
	if self:GetStackCount() ~= 1 then
		...
		ParticleManager:SetParticleControlEnt(self.parsee04_particle_2, 0, self.dummy, 5, "attach_hitloc", Vector(0,0,0), true)
		ParticleManager:SetParticleControlEnt(self.parsee04_particle_2, 1, self.caster, 5, "attach_hitloc", Vector(0,0,0), true)
		self:SetStackCount(1)
	end
	self.parsee04_particle_3 = ParticleManager:CreateParticle(particle_3, PATTACH_CUSTOMORIGIN, self.dummy)
	ParticleManager:SetParticleControlEnt(self.parsee04_particle_3, 0, self.target, 5, "attach_hitloc", Vector(0,0,0), true)
	ParticleManager:SetParticleControlEnt(self.parsee04_particle_3, 1, self.dummy, 5, "attach_hitloc", Vector(0,0,0), true)
...
function modifier_ability_thdots_parsee04_dummy:OnIntervalThink()
	...
	--吸血生命
	local life_steal = self.life_steal * FrameTime()
	local damage_tabel = {
		victim 			= self.target,
		damage 			= life_steal,
	...
	if self.target and not self.target:IsNull() and self.target:IsAlive() then
		self.caster:Heal(life_steal, self.caster)
	end
	-- SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,self.caster,life_steal,nil)
	...
	local target_distance = (targetLoc - self.dummy:GetOrigin()):Length2D()
