...
function OnTensi02SpellStart(keys)
	...
	if(keys.ability:GetContext("ability_tensi_02_reset")==TRUE)then
		...
		caster:RemoveModifierByName("modifier_tensi02_ready")
		local resetTime = keys.AbilityMulti/(caster:GetPrimaryStatValue())
		Timer.Wait 'ability_tensi_02_reset_timer' (resetTime,
			function()
				keys.ability:SetContextNum("ability_tensi_02_reset",TRUE,0)
				keys.ability:ApplyDataDrivenModifier(caster,caster,"modifier_tensi02_ready",{duration = -1})
			end
		)
	else
		return
...
function OnTensi02SpellRandom(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	if FindTelentValue(caster,"special_bonus_unique_tensi_2")==0 then 
		return
	else	
		if caster:IsRealHero() then
			local telentdamage = FindTelentValue(caster,"special_bonus_unique_tensi_1") * caster:GetStrength()
...
function OnTensi03Passive(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	caster:Heal(keys.BounsHealth+FindTelentValue(caster,"special_bonus_unique_tensi_3"), caster)
	caster:GiveMana(keys.BounsMana+FindTelentValue(caster,"special_bonus_unique_tensi_3"))
