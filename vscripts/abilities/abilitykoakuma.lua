...
function OnKoakuma01SpellJump( keys )
	...
	local bounce_targets = FindUnitsInRadius(caster:GetTeamNumber(), center, nil, ability.bounceRange, iTeam, iType, iFlag, 0, false)
	...
	if ability.projectileTo == nil then
		--find self
		bounce_targets = FindUnitsInRadius(caster:GetTeamNumber(), center, nil, ability.bounceRange, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, iFlag, FIND_CLOSEST, false)
		for _,v in pairs(bounce_targets) do
...
function OnKoakuma02SpellJump( keys )
	...
	-- find enemy hero
	local bounce_targets = FindUnitsInRadius(caster:GetTeamNumber(), targetLoc, nil, ability.bounceRange, iTeam, iType, iFlag, 0, false)
	...
	if ability.projectileTo == nil then
		--find self
		bounce_targets = FindUnitsInRadius(caster:GetTeamNumber(), targetLoc, nil, ability.bounceRange, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, iFlag, FIND_CLOSEST, false)
		for _,v in pairs(bounce_targets) do
			if v == caster:GetOwner() and v ~= target then
				ability.projectileTo = v
				break
	...
	local vec_caster = caster:GetOrigin()
		if caster:GetHealth() == 0 and caster:IsRealHero() and caster:HasModifier("modifier_item_wanbaochui") then
			...
			local targets = FindUnitsInRadius(
						...
						nil,					--find entity
						wanbaochui_damage_radius,		--find radius
			...
			for _,v in pairs(targets) do
				...
				local deal_damage=v:GetMaxHealth()*wanbaochui_damage_percent
				local damage_table = {
