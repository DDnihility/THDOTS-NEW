...
modifier_ability_thdots_chenEx_telent_4 = modifier_ability_thdots_chenEx_telent_4 or {}  --天赋监听
LinkLuaModifier("modifier_ability_thdots_chenEx_telent_4","scripts/vscripts/abilities/abilitychen.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_chenEx_telent_4:IsHidden() 		return true end
function modifier_ability_thdots_chenEx_telent_4:IsPurgable()		return false end
function modifier_ability_thdots_chenEx_telent_4:RemoveOnDeath() 	return false end
function modifier_ability_thdots_chenEx_telent_4:IsDebuff()		return false end
...
function modifier_ability_thdots_chen02_target:OnIntervalThink()
	...
	if self.wait_time <= 0.03 or not self.target:IsAlive() then
		if self:GetStackCount() == 1 then
			...
			--沉默
			UtilSilence:UnitSilenceTarget(self.caster,self.target,self.duration)
		end
...
function ability_thdots_chen04:OnSpellStart()
	if not IsServer() then return end
	self.caster 						= self:GetCaster()
	self.duration 						= self:GetSpecialValueFor("duration") + FindTelentValue(self.caster,"special_bonus_unique_cheng_2")
...
function modifier_ability_thdots_chen04:GetModifierPreAttack_BonusDamage()
	local attack_bonus = self:GetAbility():GetSpecialValueFor("attack_bonus")
	if self:GetParent():HasModifier("modifier_ability_thdots_chenEx_telent_4") then
		attack_bonus = attack_bonus + 60
	end
	return attack_bonus * self:GetStackCount()
end
