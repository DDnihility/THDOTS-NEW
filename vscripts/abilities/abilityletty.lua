...
function modifier_ability_thdots_letty03_debuff:GetModifierMoveSpeedBonus_Percentage()
	...
	local d = self:GetAbility():GetSpecialValueFor("decrease_speed") * self:GetStackCount()
	if not self:GetParent():IsRangedAttacker() then
		d = d * self:GetAbility():GetSpecialValueFor("melee_percent")/100
	end
		return d 
		-- else
...
function modifier_ability_thdots_letty04_debuff:OnCreated()
	if not IsServer() then return end
	self.attak_speed = self:GetParent():GetDisplayAttackSpeed() * self:GetAbility():GetSpecialValueFor("decrease_aspeed") / 100 * (1 - self:GetParent():GetStatusResistance())
	-- self:GetParent():EmitSound("hero_Crystal.freezingField.wind")
	--天赋禁疗
...
function modifier_ability_thdots_letty04_debuff:GetModifierMoveSpeedBonus_Percentage()
	...
	then 
		...
		return self:GetAbility():GetSpecialValueFor("decrease_mspeed") * (1 - self:GetParent():GetStatusResistance())
	end
end
