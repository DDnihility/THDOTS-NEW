...
function ability_thdots_yorihime_03:OnSpellStart()
	...
	target:FindModifierByName("modifier_shield_buff").shield_remaining = MaxHealth + strength
end
...
function ability_thdots_yorihime_ultimate:OnSpellStart()
	...
	local allies = FindUnitsInRadius(
		...
		caster:GetOrigin(),		
				   nil,					
				   self:GetSpecialValueFor("ult_radius")  + FindTelentValue(caster,"special_bonus_radius_99999"),		
				   DOTA_UNIT_TARGET_TEAM_FRIENDLY,
				   DOTA_UNIT_TARGET_HERO,
				   0, FIND_CLOSEST,
				   false
	...
	for k,v in pairs(allies) do
		if v:HasModifier("modifier_illusion")==false then
			...
			local strength = caster:GetStrength() * FindTelentValue(caster,"special_bonus_shield_strength")
			if v:HasModifier("modifier_shield_buff") then
				v:RemoveModifierByName("modifier_shield_buff")
			end
			v:AddNewModifier(caster,ability3,"modifier_shield_buff",{duration = duration })
			v:FindModifierByName("modifier_shield_buff").shield_remaining = MaxHealth + strength
			local target = v
