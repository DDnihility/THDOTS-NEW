...
function GetNueExtraDamage(caster)
	...
	local basedamage = ability:GetSpecialValueFor("damage") + FindTelentValue(caster,"special_bonus_unique_nue_1")
...
function OnNue01AttackLanded(keys)
	...
	local DamageTable = {
			...
			attacker = caster, 
			ability = keys.ability,
			damage = GetNueExtraDamage(caster) * keys.damage_percent, 
			damage_type = keys.ability:GetAbilityDamageType(), 
			damage_flags = keys.ability:GetAbilityTargetFlags()
	}
	...
	caster:SetContextThink(DoUniqueString("OnNue04SpellThink"), 
		function()
		    ...
		    for k,v in pairs(targets) do
				...
				UtilStun:UnitStunTarget(caster,v,keys.stun_duration+FindTelentValue(caster,"special_bonus_unique_nue_2"))	
		    	UnitDamageTarget(damage_table)
			end
