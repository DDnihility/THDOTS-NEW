...
function THD_URD(plyid,plyhd)
	...
	if Team1[plyid] ~= nil then
		...
		if Team1_count == 0 then
			Team1_count = 1
			GameRules:SendCustomMessage("<font color='#00FFFF'>博丽神社发起了投降,输入-urd投票投降，当前票数:"..Team1_count.."/4</font>", DOTA_TEAM_GOODGUYS, 0)
	...
	if Team2[plyid] ~= nil then
		...
		if Team2_count == 0 then
			Team2_count = 1
			GameRules:SendCustomMessage("<font color='#00FFFF'>守矢神社发起了投降,输入-urd投票投降，当前票数:"..Team2_count.."/4</font>", DOTA_TEAM_BADGUYS, 0)
		...
		function()
			...
			if time < TIME_DURATION then
				...
				if count > Team1_count then
					print("tou piao  + 1")
					Team1_count = count
					GameRules:SendCustomMessage("<font color='#00FFFF'>同意投降，当前票数:"..Team1_count.."/4</font>",DOTA_TEAM_GOODGUYS, 0)
				end
				if Team1_count >= 4 then
					print("SS shengli")
					GameRules:SendCustomMessage("<font color='#00FFFF'>博丽神社投降，游戏将在10秒后结束</font>", 0, 0)
				...
				print("end")
				GameRules:SendCustomMessage("<font color='#00FFFF'>博丽神社投降失败，当前票数:"..Team1_count.."/4。2分钟以内无法再次发起投降</font>",DOTA_TEAM_GOODGUYS, 0)
				FLAG1 = false
				FLAG1_interval()
				for _,v in pairs(Team1) do
					v.TF = 0
		...
		function()
			...
			if time < TIME_DURATION then
				...
				if count > Team2_count then
					print("tou piao  + 1")
					Team2_count = count
					GameRules:SendCustomMessage("<font color='#00FFFF'>同意投降，当前票数:"..Team2_count.."/4</font>",DOTA_TEAM_BADGUYS, 0)
				end
				if Team2_count >= 4 then
					print("BL shengli")
					WINER_TEAM = DOTA_TEAM_GOODGUYS
					GameRules:SendCustomMessage("<font color='#00FFFF'>守矢神社投降，游戏将在10秒后结束</font>", 0, 0)	
					local end_time = 0
				...
				print("end")
				GameRules:SendCustomMessage("<font color='#00FFFF'>守矢神社投降失败，当前票数:"..Team2_count.."/4。2分钟以内无法再次发起投降</font>",DOTA_TEAM_BADGUYS, 0)
				FLAG2 = false
				FLAG2_interval()
				for _,v in pairs(Team2) do
					v.TF = 0
