...
Shuffle_Pressed = function (keys) --洗牌，GetTHDPlayerRank获取了10组排列顺序，每点击一次洗牌按钮会优先抽取分差最小的组合序号TrueList
...
else --RankingDiscard模式随机洗牌
	local List = {}
	local player = {plyhd = nil, SteamID = nil}
	for i=1, PlayerResource:GetPlayerCount() do
		local plyhd = PlayerResource:GetPlayer(i-1)
		if plyhd then
			player = {plyhd = plyhd,SteamID = PlayerResource:GetSteamID(plyhd:GetPlayerID())}
	...
	while total > 0 do
		...
		table.insert(PlayerList,List[i])
		List[i]=List[total]
		total = total - 1
	end
	...
	for i=1,#PlayerList do
		if PlayerList[i] == nil then break end
		-- print("NOTEAM")
		-- print(PlayerList[i].plyhd:GetPlayerID())
		PlayerResource:SetCustomTeamAssignment(PlayerList[i].plyhd:GetPlayerID(),DOTA_TEAM_NOTEAM)
	end
	for i=1,#PlayerList do
		if PlayerList[i] == nil then break end
		-- print(PlayerList[i].plyhd:GetPlayerID())
		-- print(team)
		PlayerResource:SetCustomTeamAssignment(PlayerList[i].plyhd:GetPlayerID(),team)
